    ...
    <div>
      <v-container>
          <v-row>
              ...
              <v-col cols="9">
                  <div>
                      <v-alert class="mt-3" v-model="successAlert" dismissible dense type="success">
                          Successfully Added Shipping Address
                      </v-alert>
  
                      <v-alert class="mt-3" v-model="defaultAlert" dismissible dense type="info">
                          Default Shipping Address is Now Change
                      </v-alert>

                      <v-alert class="mt-3" v-model="updateAlert" dismissible dense type="info">
                      ...
                      <v-tab-item value="tab-1">
                          ...
                          <v-card-text>
                              ...
                              <template v-slot:item.userAddress = "{ item }">
                                  {{ item.userAddress.userAddressStreet1 }} {{ item.userAddress.userAddressStreet2 }}, {{ item.userAddress.userAddressCity }}, {{ item.userAddress.userAddressState }}
                              </template>
                              <template v-slot:item.actions = "{ item }">
                                  ...
                                  x-small
                                  elevation="2"
                                  @click="deleteDialog = true;toDeleteShipAddress = item.userAddress;deleteShippingId=item.userShippingId"
                              ...
                              <v-dialog v-model="deleteDialog" width="400">
                                    <v-card>
                                        ...
                                        <v-row dense>
                                            ...
                                            <v-col cols="7">{{ toDeleteShipAddress.userAddressId }}</v-col>
                                            <v-col cols="3" class="font-weight-bold">Address</v-col>
                                            <v-col cols="7">{{ toDeleteShipAddress.userAddressStreet1 }} {{ toDeleteShipAddress.userAddressStreet2 }} {{ toDeleteShipAddress.userAddressCity }} {{ toDeleteShipAddress.userAddressState }}</v-col>
                                        </v-row>
                                        </v-card-text>
                                        <v-card-actions class="justify-end">
                                        <v-btn color="red" dark @click="deleteShippingAddress()">Delete</v-btn>
                                        </v-card-actions>
                                    </v-card>
                                </v-dialog>
  
                              <v-btn color="primary" @click="saveDefault">Save Default</v-btn>
                          </v-card-text>
                          </v-card>
                      </v-tab-item>
  ...
  export default {
      ...
      methods: {
          async listOfShippingAddresses() {
              const resp = await utils.http.get("/api/user/shipping/list?currentUserId=" + this.loginUser.id);
              if (resp.status === 200) {
            ...
            this.updateAlert = false;
              if (this.$refs.shippingForm.validate()) {
                  if (this.userShippingId == null) {
                    const resp = await utils.http.post("/api/user/shipping/add?currentUserId=" + this.loginUser.id, {
                    ...
                    const resp = await utils.http.put("/api/user/shipping/update?currentUserId=" + this.loginUser.id, {
                      userAddressId: this.userShippingId,
                      userAddressName: this.userShippingName,
                      userAddressStreet1: this.userShippingStreet1,
                      userAddressStreet2: this.userShippingStreet2,
          ...
          async deleteShippingAddress() {
            ...
            // console.log(this.deleteShippingId)
              const resp = await utils.http.del("/api/user/shipping/remove?userShippingId=" + this.deleteShippingId + "&currentUserId=" + this.loginUser.id);
              if (resp.status === 200) {
                  await this.listOfShippingAddresses();
          ...
          async saveDefault() {
            ...
            this.updateAlert = false;
            this.defaultAlert = false;
            const resp = await utils.http.post("/api/user/shipping/default?userShippingId=" + this.selected + "&currentUserId=" + this.loginUser.id);
            if (resp.status === 200) {
                const data = await resp.json();
                if (data) {
