        ...
        <v-col cols="10">            
          <v-form class="" ref="bookForm" v-model="bookForm">
            <v-row>
              ...
              :counter="255"
              label="Title"
              :rules="[(v) => !!v || 'Required',(v) => (v && v.length <= 255) || 'Title must be less than 255 characters.']">
              </v-text-field>
...
export default {
  ...
  data() {
      return {
          ...
          categoryList: ["Management","Fiction", "Engineering", "Programming", "Arts and Literature"],
  ...
  methods: {
      async addBook() {
          if (this.$refs.bookForm.validate()) {
              ...
              const respBookImage = await utils.http.postMedia("/api/admin/book/file/create", this.bookImage, this.bookImage.type);
