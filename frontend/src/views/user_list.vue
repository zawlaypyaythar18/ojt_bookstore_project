...
export default {
 ...
 methods: {
    ...
    async updateStatus() {
        ...
        const resp = await utils.http.put("/api/admin/user/status/update?userId=" + this.toUpdateStatus.id + "&status=" + this.toUpdateStatus.userStatus);
        if (resp && resp.status === 200) {
            await this.getUserList();
    ...
    async deleteUser() {
          const resp = await utils.http.del(
              "/api/admin/user/delete/" + this.toDeleteUser.id
          );
          if (resp.status === 200) {
