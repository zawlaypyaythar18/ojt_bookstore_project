...
export default {
 ...
 methods: {
    async getUserList() {
        const resp = await utils.http.get("/admin/userList");
    ...
    async getUserStatus() {
        const resp = await utils.http.get("/admin/userStatus");
        if (resp && resp.status === 200) {
            const data = await resp.json();
            if (data) {
                // console.log(data);
    ...
    async updateStatus() {
        ...
        const resp = await utils.http.put("/admin/user/updateStatus?userId=" + this.toUpdateStatus.id + "&status=" + this.toUpdateStatus.userStatus);
        if (resp && resp.status === 200) {
            await this.getUserList();
