                      ...
                      <v-tab-item value="tab-1">
                          ...
                          <v-card-text>
                              ...
                              <template v-slot:item.userBilling = "{ item }">
                                  {{ item.userBilling.userAddress.userAddressStreet1 }} {{ item.userBilling.userAddress.userAddressStreet2 }}, {{ item.userBilling.userAddress.userAddressCity }}, {{ item.userBilling.userAddress.userAddressState }}
                              </template>
                              <template v-slot:item.actions = "{ item }">
                                  ...
                                  x-small
                                  elevation="2"
                                  @click="deleteDialog = true;toDeleteCreditCard = item;"
                                ...
                                <v-dialog v-model="deleteDialog" width="400">
                                    <v-card>
                                        ...
                                        <v-row dense>
                                            ...
                                            <v-col cols="3" class="font-weight-bold">Card Name</v-col>
                                            <v-col cols="7">{{ toDeleteCreditCard.cardName }}</v-col>
                                        </v-row>
                                        </v-card-text>
                                        <v-card-actions class="justify-end">
                                        <v-btn color="red" dark @click="removeCreditCard(toDeleteCreditCard)">Delete</v-btn>
                                        </v-card-actions>
                                    </v-card>
                                </v-dialog>
  ...
  export default {
      ...
      data() {
          return {
              ...
              holderName: "",
              cardNumber: null,
              expiryMonthList: ["01","02","03","04","05","06","07","08","09","10","11","12"],
              expiryMonth: "",
              expiryYearList: ["2022","2023","2024","2025","2026","2027","2028","2029","2030"],
              expiryYear: "",
              cvc: null,
          }
      },
      ...
      methods: {
          async listOfCreditCards() {
              const resp = await utils.http.get("/api/user/card/list?currentUserId=" + this.loginUser.id);
              if (resp.status === 200) {
          ...
          async addNewCreditCard() {
              if (this.$refs.billingForm.validate()) {
                ...
                if (this.userBillingId == null) {
                    const resp = await utils.http.post("/api/user/card/add?currentUserId=" + this.loginUser.id, {
                    ...
                    userPaymentMethod: {
                        ...
                        const resp = await utils.http.put("/api/user/card/update?currentUserId=" + this.loginUser.id, {
          ...
          async removeCreditCard(item) {
            ...
            this.updateAlert = false;
            this.defaultAlert = false;
              const resp = await utils.http.del("/api/user/card/remove?id=" + item.id + "&currentUserId=" + this.loginUser.id);
              if (resp.status === 200) {
                await this.listOfCreditCards();
          ...
          async saveDefault() {
            this.successAlert = false;
            this.updateAlert = false;
            const resp = await utils.http.post("/api/user/card/default?userPaymentId=" + this.selected + "&currentUserId=" + this.loginUser.id);
            if (resp.status === 200) {
                const data = await resp.json();
                if (data) {
